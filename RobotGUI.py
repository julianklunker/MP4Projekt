...
class RobotGUI:
    ...
    # ── Right: info panels ───────────────────────────────────────────────
        ...
        # Robot status
        status_frame = tk.LabelFrame(right, text="Robot", bg="#1e1e1e",
                                     fg="#aaaaaa", font=("Helvetica", 10))
        status_frame.pack(fill=tk.X, pady=(0, 10))

        self.robot_item_label = tk.Label(status_frame, text="Current item: None",
                                         bg="#1e1e1e", fg="white",
                                         font=("Helvetica", 11))
    ...
    def poll_queue(self):
        while not self.data_queue.empty():
            ...
            if "robot_item" in data:
                item = data["robot_item"]
                if item:
                    color, robot_x, t = item
                    text = f"Current item: {color}  X={robot_x:.1f}mm"
                else:
                    text = "Current item: None"
                self.robot_item_label.config(text=text)
